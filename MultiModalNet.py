...
class VisionNet(nn.Module):
    def __init__(self,vision_model,optimizer,vision_loss,get_pretrained,**kwargs):
        ...
        if get_pretrained:
            self.__S__ = VisionNetModel(**kwargs,pretrained = True)
            print("pretrained ResNet is loaded")
        else:
            self.__S__ = VisionNetModel(**kwargs,pretrained=False)
...
class AudioNet(nn.Module):
    def __init__(self,audio_model,optimizer,audio_loss,get_pretrained,**kwargs):
        ...
        if get_pretrained:
            self.__S__ = AudioNetModel(**kwargs,pretrained = True)
            print("pretrained ResNet is loaded")
        else:
            self.__S__ = AudioNetModel(**kwargs,pretrained=False)
            print("not pretrained model is loaded")
...
class ModelTrainer(object):
    ...
    def evaluateFromList(self,test_path,audio_path,frame_path,audio_ext,image_ext,random_sample,clipping_duration,nDataLoaderThread, transform,batch_size,**kwargs):
        ...
        test_dataset =  SpectroLoader.meta_Dataset(audio_path,frame_path,audio_ext,image_ext,test_path,transform,random_sample=True,clipping_duration=clipping_duration)
